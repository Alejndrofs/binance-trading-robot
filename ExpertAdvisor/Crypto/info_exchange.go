...
func NewExchangeInfo(expert *ExpertAdvisorCrypto) *ExchangeInfo {
	Ex := &ExchangeInfo{
		Expert:          expert,
...
func (ea *ExchangeInfo) FindSymbolByAsset(base Asset, prof Asset) (*SymbolInfo, error) {
	for i := range ea.Expert.Account.SymbolsList {
		if (ea.Expert.Account.SymbolsList[i].BaseAsset.AssetName == base.AssetName && ea.Expert.Account.SymbolsList[i].ProfAsset.AssetName == prof.AssetName) || (ea.Expert.Account.SymbolsList[i].BaseAsset.AssetName == prof.AssetName && ea.Expert.Account.SymbolsList[i].ProfAsset.AssetName == base.AssetName) {
			return ea.Expert.Account.SymbolsList[i], nil
		}
	}
	return &SymbolInfo{}, errors.New("find symbol by asset: symbol not found")
}

func (ea *ExchangeInfo) FindSymbolByName(base string, prof string) (*SymbolInfo, error) {
	for i := range ea.Expert.Account.SymbolsList {
	...
	return &SymbolInfo{}, errors.New("find symbol by name: symbol not found")
}

func (ea *ExchangeInfo) ConvertFactor(from, to string) (float64, error) {
	ea.Expert.Debug = true
	if from == to {
		return 1, nil
	}
...
func (ea *ExchangeInfo) SymbolInfoBool(symbol string, x ENUM_SYMBOL_INFO_BOOL) (bool, error) {
	if !ea.ExchangeInfoSet {
		exchange, err := ea.Expert.Client.NewExchangeInfoService().Do(context.Background())
		if err == nil {
			ea.Info = exchange
	...
	if x == SYMBOL_INFO_NOTIONAL_APPLY {
		return ea.MinNotionalFilterBool(symbol, SYMBOL_INFO_NOTIONAL_APPLY)
	}
	...
	for _, s := range ea.Info.Symbols {
		if s.Symbol == symbol {
			switch x {
			case "icebergAllowed":
				return s.IcebergAllowed, nil
			case "ocoAllowed":
				return s.OcoAllowed, nil
			case "isSpotTradingAllowed":
				return s.IsSpotTradingAllowed, nil
			case "isMarginTradingAllowed":
				return s.IsMarginTradingAllowed, nil
			}
		}
	}

	return false, errors.New("symbol info bool error: filter not found")
}
...
func (ea *ExchangeInfo) PercentPriceFilterInt(symbol string, f ENUM_SYMBOL_INFO_INT) (int, error) {
	...
	for i := range ea.Info.Symbols {
		if ea.Info.Symbols[i].Symbol == symbol {
			for _, filter := range ea.Info.Symbols[i].Filters {
				if filter["filterType"].(string) == string(binance.SymbolFilterTypePercentPrice) {
					if f == "avgPriceMins" {
...
func (ea *ExchangeInfo) IcebergPartsFilter(symbol string, f ENUM_SYMBOL_INFO_INT) (int, error) {
	...
	for i := range ea.Info.Symbols {
		if ea.Info.Symbols[i].Symbol == symbol {
			for _, filter := range ea.Info.Symbols[i].Filters {
				if filter["filterType"].(string) == string(binance.SymbolFilterTypeIcebergParts) {
					if f == "limit" {
...
func (ea *ExchangeInfo) MarketLotSizeFilter(symbol string, f ENUM_SYMBOL_INFO_DOUBLE) (float64, error) {
	...
	for i := range ea.Info.Symbols {
		if ea.Info.Symbols[i].Symbol == symbol {
			for _, filter := range ea.Info.Symbols[i].Filters {
				if filter["filterType"].(string) == string(binance.SymbolFilterTypeMarketLotSize) {
					if f == "maxQty" {
						if i, ok := filter["maxQty"]; ok {
							return strconv.ParseFloat(i.(string), 64)
						}
...
func (ea *ExchangeInfo) MaxNumAlgoOrdersFilter(symbol string, f ENUM_SYMBOL_INFO_INT) (int, error) {
	...
	for i := range ea.Info.Symbols {
		if ea.Info.Symbols[i].Symbol == symbol {
			for _, filter := range ea.Info.Symbols[i].Filters {
				if filter["filterType"].(string) == string(binance.SymbolFilterTypeMaxNumAlgoOrders) {
