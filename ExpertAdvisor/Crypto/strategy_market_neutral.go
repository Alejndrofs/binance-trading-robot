...
func NewMarketNewtralStrategy(ea *ExpertAdvisorCrypto, assets []string) (*MarketNewtralStrategy, error) {
	...
	for i := range assets {
		if ea.MainAsset != assets[i] {
			...
			sym, err := ea.Exchange.FindSymbolByAsset(*main_asset, *activo)
			if err != nil {
...
/*
func NewSyntheticMarketNewtralStrategy(ea *ExpertAdvisorCrypto,assetA string,assetB string,grid float64,_range float64) (*MarketNewtralStrategy, error) {
	if assetA != assetB {
		strategy := &MarketNewtralStrategy{
			Expert: ea,
		}
...
func (strategy *MarketNewtralStrategy) Ontimer() {
	...
	for i := range strategy.Performance {
		if strategy.Performance[i].Initialized {
			newBar, _ := strategy.Performance[i].UpdateRenko()
