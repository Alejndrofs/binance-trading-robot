...
func (cq *ConversionManager) newTrade(symbol *SymbolInfo, buy bool, price float64) *Trade {
	...
	var isBuy string = "false"
	if buy {
		isBuy = "true"
	}

	cq.Expert.UpdateTradesLog(symbol.SymbolName + " buy:" + isBuy + " price:" + fmt.Sprintf("%f", price))

	cq.HistoryTrades = append(cq.HistoryTrades, trade)
	return trade
}

func (cq *ConversionManager) isPosition(symbol *SymbolInfo, buy bool, price float64) bool {
	trade := &Trade{
		TradeID: lastID,
		Buy:     buy,
		Price:   price,
	...
	for i := 0; i < len(cq.Queue); i++ {
		...
		if err == nil {
			if cq.Queue[i].Buy && cq.Queue[i].Symbol.ProfAsset.IsEnoughMoney(cq.Queue[i].Volume*conv) {
				...
				if orderID == -1 {
					return ConvertVolumeReturn{}, errors.New("open buy error, orderID = -1")
				}
			...
			return ConvertVolumeReturn{}, errors.New("convert factor error")
		}
	}
...
func (cq *ConversionManager) VirtualExecuteQueue() (ConvertVolumeReturn, error) {
	for i := len(cq.Queue) - 1; i >= 0; i-- {
		if i == len(cq.Queue)-1 {
			cq.Queue[i].Symbol.NormalizeLot(cq.Queue[i].Volume)
		} else {
	...
	for i := 0; i < len(cq.Queue); i++ {
		...
		if err == nil {
			...
			return ConvertVolumeReturn{}, errors.New("virtual execute queue: convert factor error")
		}
	}
	var lastAsset string
	var vol float64
...
func (ea *ConversionManager) ConvertFactorVolume(volume float64, from, to string) (ConvertVolumeReturn, error) {
	...
	if ea.Expert.AccountType == ACCOUNT_REAL {
		ret, err := conversionTree.Conversions[minimumTreeIndex].ExecuteQueue()
		if err == nil {
			ea.newTrade(Symbol, Buy, Price)
			return ret, nil
		} else {
			return ConvertVolumeReturn{}, errors.New("convert factor volume error")
		}
	} else {
		ret, err := conversionTree.Conversions[minimumTreeIndex].VirtualExecuteQueue()
		if err == nil {
			ea.newTrade(Symbol, Buy, Price)
			return ret, nil
		} else {
